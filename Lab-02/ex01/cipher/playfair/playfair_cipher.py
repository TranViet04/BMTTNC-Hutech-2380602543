class PlayFairCipher:
    ...
    def find_letter_coord(self, matrix, letter):
    ...
    def playfair_encrypt(self, plaintext, matrix):
        # Chuyen doi chu J thanh I, trong van ban dau vao
        plain_text = plaintext.replace("J", "I")
        ...
        for i in range(0, len(plain_text), 2):
            ...
            if len(pair) == 1:
                pair += "X"
            row1, col1 = self.find_letter_coord(matrix, pair[0])
            row2, col2 = self.find_letter_coord(matrix, pair[1])
            if row1 == row2:
